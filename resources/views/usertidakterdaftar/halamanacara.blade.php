...
@if ($indexkategoriacara)   
        ...
        <h1>{{ $indexkategoriacara->judul }}</h1>
        <p class="fw-medium">Dibuat pada {{ $indexkategoriacara->created_at->formatLocalized('%A, %d %B %Y') }}</p>
        
        <div class="text-center py-5">
        <img src="/image/{{ $indexkategoriacara->image }}" width="500px" alt="" />
        </div>
        <p>
        {{ $indexkategoriacara->keterangan }}
        </p>
        </div>
@else
    <p>Data not found.</p>
